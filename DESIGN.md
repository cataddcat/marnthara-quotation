...
- **Radii:** controls `rounded-lg`/`rounded-xl`; cards/containers `rounded-xl`. Avoid `rounded-2xl`+ on controls.
  ...
  *(**EEERT คมชัด** (2026-06-20): EEERT overrides the radius tokens to a tighter "engineered" scale —
  `rounded-lg`→**4px** controls, `rounded-xl`→**6px** cards/modal — plus hardened cobalt borders and
  **opaque de-frosted chrome** (header/dock/modal header drop their `backdrop-blur` via `eeert:` overrides).
