...
>
> **กฎของไฟล์นี้:** ตอบคำถามเดียว — *"X คืออะไร"*. **อนุญาต** identifier / §-pointer ที่ map ศัพท์→concept ในโค้ดได้ (เช่น Style = key ใน `laborCosts`). **ห้าม**: ค่า volatile (ตัวเลขสูตร/ขนาด/ค่าที่ owner อื่นถือ — ชี้ไป owner แทน) · bug history · dated worklog · rationale ยาว (อยู่ HANDOFF.md / git log แล้ว)
>
> **ขอบเขต (เพิ่ม 2026-06-19):** ไฟล์นี้เป็นเจ้าของ **"app copy / microcopy"** ด้วย — *คำที่ผู้ใช้เห็นในแอป* และ **คำต้องห้าม** (ตาราง §6) ตัดสินที่นี่. แต่ **น้ำเสียง** (พูดกับเจ้าของร้านไม่ใช่ลูกค้า · เปิดเผยทุน/กำไร) = ทิศทางจาก PRINCIPLES §2.5; และ **decision เฉพาะเรื่องคำ** (เช่น "กำไร" → "ส่วนต่างจากทุนที่รู้") บันทึกที่ MEMORY (ภายใต้ร่มเจ้าของ = ไฟล์นี้)
...
สูตรทั่วไป (ตาไก่) เพิ่มขึ้น (ม่านลอนใช้จำนวนขาเท่าเดิม — ดู [[Brackets]])
...
### Brackets
ขาแขวน rail จำนวนคิดจากความกว้าง **สูตร/ค่าจริงขึ้นกับ style — เจ้าของที่ `FORMULAS.materials` (อ่านได้ที่ [[Formula Docs]])**. หลักที่ต้องรู้:
- **ม่านลอน:** DOUBLE ใช้จำนวนขา **เท่าเดิม** (ขา 2 ชั้น ไม่คูณเพิ่ม)
- **ม่านตาไก่ / สูตรทั่วไป:** DOUBLE **เพิ่มขึ้น** (รับน้ำหนัก 2 ชั้น)
- **ม่านแป๊บ (rod):** จำนวนคงที่ต่อชุด
...
### Formula / Multiplier
ค่าคงที่ที่ใช้คูณในการคำนวณ — เช่น ปริมาณผ้าต่อ style = width × multiplier ของ style นั้น เป็น **compile-time
constant** ใน `src/config/formulas.ts` (แก้โดยแก้โค้ด ไม่ปรับใน UI); ดูค่าปัจจุบันแบบอ่านอย่างเดียวได้ที่ [[Formula Docs]]

### Yard Conversion
ค่าแปลงเมตรเป็นหลา ใช้แปลงต้นทุนเมตริก → หลา (ภาษาช่าง); ค่าจริง = `src/config/formulas.ts`
...
### Formula Docs
หน้า **อ่านอย่างเดียว** แสดง multiplier/offset ปัจจุบัน (ค่าจริงมาจาก `src/config/formulas.ts`) เปิดผ่าน main menu
*(สูตรเป็น compile-time แก้ในโค้ด ไม่ใช่ UI — ดู anti-term "Formula Studio" §6)*
...
### localStorage
ที่เก็บข้อมูลถาวรในเบราว์เซอร์ของอุปกรณ์ ข้อมูล quotation, vault, settings อยู่ที่นี่ (local-first) ไม่มี custom DB server ของเราเอง — ถ้าเปิด [[Sync]] ข้อมูลจะขึ้น Firestore ด้วย (ดู §8)
...
| Formula Studio | [[Formula Docs]] (อ่านอย่างเดียว) / `formulas.ts` | สูตรเป็น compile-time แก้ในโค้ด ไม่ใช่ UI |
...
*(ดู [[Sync]] §8 + HANDOFF §12; ระบุให้แม่น ห้ามพูดลอย ๆ ว่า "ไม่มี")*
