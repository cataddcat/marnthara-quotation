...
**▶ Session หน้าเริ่มตรงนี้ได้:** 5 ข้อ kept ข้างบน (ถ้า owner เอา) + **Stage 3 density (🟡 owner) ด้านล่าง**
+ surface ที่ยังไม่ audit (print parts — typography-exempt · CustomerModal · ProjectOverviewModal ·
LookbookModal · ProductionSettingsModal). อ่านคู่กับ DESIGN.md.

---

## 🆕 Stage 3 — surface ใหม่ (2026-06-17) — audit + แก้แล้ว (gate เขียว · รอ owner commit ผ่าน GUI)

ตรวจ 4 surface ที่ Stage 2 ยังไม่แตะ: **MainMenuModal · SmartNavigator · CodeDetailModal ·
MaterialSummaryModal (= "Catalog")** + แตะ **Modal.tsx** primitive (title). gate เขียว (lint 0 ·
build · vitest 667). ทั้งหมดเป็น className-only — ไม่กระทบ logic/test.

**✅ แก้แล้ว (🔴/🟢 — กฎ-backed, ปลอดภัย):**
| # | file:line | ของเดิม | แก้เป็น | กฎ |
|---|---|---|---|---|
| 33 | MainMenuModal:93 (label ไทยเมนู) | `text-sm leading-tight` | `leading-snug` | §1 ไทยไม่ tight |
| 34 | MainMenuModal:162 (ชื่อร้าน) | `leading-tight` | `leading-normal` | §1 (ตรง RoomCard:350) |
| 35 | MainMenuModal:288 (ปิดโหมดทีม) | `min-h-[36px]` | `min-h-[44px]` | §3 ทัช |
| 36 | MainMenuModal:270 (KeyRound) | กว้าง ~32px | `min-w-[44px]`+center | §3 ทัช (กว้าง) |
| 37 | SmartNavigator:108 (ชื่อห้อง) | `leading-tight` | `leading-normal` | §1 ไทย |
| 38 | CodeDetailModal:139 (ปุ่มเปิดคลัง) | `h-9` (36) `text-xs` | `h-11` (44) `text-sm` | §3 ทัช + §1 ปุ่ม≥14 |
| 39 | CodeDetailModal:147,157,163 | lucide ไม่มี stroke | `strokeWidth={1.5}` | §4 |
| 40 | MaterialSummaryModal:1299 (pills มือถือ) | `bg-primary` active | `bg-foreground text-background` + `min-h-[44px]` | §2 primary=CTA only · §3 ทัช |
| 41 | Modal.tsx:219 (title ทุก modal) | `leading-tight` | `leading-snug` | §1 ไทย (primitive — single-line truncate, +2px, no layout shift) |
| 42 | MaterialSummaryModal InlineCostEditor (L48 — primitive, 4 usages) | ทัช ~24px (`py-0.5`) | `min-h-[44px]` ทั้ง view+edit state | §3 ทัช (owner: ขยายเฉพาะ action หลัก) |
| 46 | strokeWidth sweep ทั้ง MaterialSummaryModal (~15 ไอคอน) | lucide เหลือ default stroke 2 | `strokeWidth={1.5}` ทุกตัวที่ขาด | §4 |

**⏳ Owner เลือก "คงไว้" (decision เคาะแล้ว 2026-06-17 — ไม่ใช่ค้าง):**
| # | file:line | เรื่อง | เหตุผลที่คงไว้ |
|---|---|---|---|
| 43 | MaterialSummaryModal row actions (258-287) | MapPin/Pencil/Trash `p-1.5` ~28px | owner: เป็น action **รอง** — ขยาย = กินแนวนอน ~48px บนจอ 390 + แถวพอง; คงแน่นเพื่อเห็นรหัสเยอะ |
| 44 | MaterialSummaryModal edit/add forms (199,415) | ปุ่ม `py-1.5` ~30px | overlay ชั่วคราว (แสดงนาน ๆ ครั้ง) นอกขอบเขตที่ owner เลือก |
| 45 | MaterialSummaryModal cards (220,553,…) | `rounded-2xl` | **ไม่ใช่ deviation** — `rounded-2xl` คือมาตรฐานการ์ดทั้งแอป (13 ไฟล์ รวม RoomCard/ItemCard/ItemSummaryCard) |

> #42 + #46 ทำแล้ว (gate เขียว · รอ owner commit). ทิศที่ owner เคาะ: **"เฉพาะที่คุ้ม + กันแถวพอง"** —
> ขยายแค่ตัวตั้งราคาทุน (action หลัก, แตะ ≥44px ทั้งคลังรหัส + การ์ดสรุปกางออก), ไอคอนแถวคงแน่น.

---

## 🆕 Stage 3.1 — quality pass เหนือ floor (2026-06-17) — hierarchy / flow / ความสวย

หลัง floor-fixes เสร็จ owner ขอ review เชิงคุณภาพ (เกิน "ผ่านกฎ") แล้วเลือกทำ 4 ข้อ. gate เขียว
(lint 0 · build · vitest 667). className/reorder ล้วน — ไม่แตะ logic/handler.

| # | จอ | เรื่อง | แก้เป็น | เลนส์ / กฎ |
|---|---|---|---|---|
| A1 | MainMenuModal | เปิดเมนูเจอ "ตั้งค่า/บัญชี" ก่อน "งาน" (ลำดับกลับหัว) | ย้ายบล็อก `bg-muted/30` (greeting+บัญชี+บทบาท+ธีม/โหมด) ลง**ใต้ 3 section งาน** เหนือ footer → **actions-first** | hierarchy/flow · §0 (สำคัญก่อน) |
| C1 | MainMenuModal | สีไอคอนเมนู decorative + ไม่สม่ำเสมอ (emerald บน "คัดลอกสรุป/ส่วนลด") | ตัด prop `accentColor`+`colors` map → ไอคอน **monochrome** (`bg-muted text-foreground`) ทุกปุ่ม · **คงแถบสีหัวหมวด 3 สี** (chunking) | ความสวย/consistency · §2 (สี=ความหมาย) · §1.7#1 monochrome-first |
| A2 | SmartNavigator | ชื่อห้อง `text-[15px]` (arbitrary นอกสเกล) | → `text-base` (16px = Title role) | consistency · §1 type scale |
| B1 | MaterialSummaryModal | สับสน "ตั้งทุนได้ 2 ที่" (FabricCard กางออก vs คลังรหัส) | เพิ่ม hint 1 บรรทัดบนแท็บ "ราคา & รหัส": *"ตั้งทุนที่นี่หรือที่การ์ดสรุปก็ได้ — เป็นค่าเดียวกัน…"* | flow/cognitive load · §0 |
| B3 | RoomSlider (focus) | โหมดโฟกัสมีแค่ "เพิ่มสินค้า" — เพิ่มห้องต้องเข้าลิ้นชัก/สลับภาพรวม | เพิ่ม **"+ แท็บใหม่"** บนแถบนำทาง: prev/next → ไอคอนล้วน (44px) · "+" วงกลมเส้นประต่อท้ายจุดห้อง → `addRoom()` แล้ว [App.tsx:49](src/App.tsx#L49) เด้งเข้าห้องใหม่. *(รอบแรกวางปุ่มเต็มกว้างใต้การ์ด — owner ตีกลับว่าผิดลำดับชั้น → redesign เป็น new-tab pattern อ้างอิงเบราว์เซอร์/Figma/iOS)* | flow · hierarchy: เพิ่มห้อง=ระดับนำทาง แยกจากเนื้อหาในห้อง |

> ผลรวม: MainMenuModal กลายเป็น reference impl chunking ที่ **actions-first** (เผื่อ DESIGN §0 อ้าง) +
> ระบบสี "สี=ความหมาย · neutral chrome only" (DESIGN §2 — เลิกใช้คำ "monochrome chrome") เนียนทั้งแอป. **รอ owner commit + eyeball มือถือจริง.**
> ข้อ review ที่ owner ยังไม่สั่งทำ (B2 การ์ดบทบาทหนา · C2 emoji หัว section · จอที่ยังไม่ audit เช่น
> LookbookModal ที่มี `text-slate-900` + leading-tight) — ค้างไว้.
...
> ⚠️ บางข้อมีโน้ต **[วัด Probe]** = ควรยืนยันด้วย DevInspector phone window 390×844 + notch sim ก่อนเคาะค่าสุดท้าย.
...
- `npm run dev` → Probe phone window 390×844 + notch sim → ยืนยันข้อ **[วัด Probe]**: #1(Thai ชน), #19(dual-scroll), #20(grid เงินล้าน), #22(legend ล้น), #25(truncate), #30(PdfPreview clip)
