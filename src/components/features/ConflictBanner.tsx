// src/components/features/ConflictBanner.tsx
...
export const ConflictBanner: React.FC = () => {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-[60] pt-safe-top px-3 pointer-events-none">
      <div className="mx-auto max-w-2xl mt-2 rounded-2xl border border-amber-300 bg-amber-50 dark:bg-amber-950/80 dark:border-amber-800 shadow-xl p-3 pointer-events-auto animate-fade-in">
        <div className="flex items-start gap-2.5">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs text-amber-800 dark:text-amber-300 mt-0.5 leading-relaxed">
              "{name}" มีเวอร์ชันใหม่กว่าจากอุปกรณ์อื่น — เลือกเก็บอันไหน (อีกอันจะถูกเก็บเป็นสำเนา ไม่หาย)
            </div>
            <div className="flex gap-2 mt-2.5">
              <button
                onClick={() => choose('load')}
                className="flex-1 min-h-[40px] rounded-lg bg-amber-600 text-white text-sm font-semibold hover:bg-amber-700 active:scale-[0.98] transition"
              >
                โหลดล่าสุด
              </button>
              <button
                onClick={() => choose('keep')}
                className="flex-1 min-h-[40px] rounded-lg border border-amber-400 text-amber-800 dark:text-amber-200 text-sm font-semibold hover:bg-amber-100 dark:hover:bg-amber-900/40 active:scale-[0.98] transition"
              >
                เก็บของฉัน
              </button>
            </div>
