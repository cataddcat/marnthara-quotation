...
- **Squircle corner system (in progress, 2026-06-29)** — เปลี่ยนวิธีทำมุมปุ่ม rounded-rect → **squircle** (superellipse) แบบ **cross-browser ทุกอุปกรณ์/ทุกธีม** (CSS `corner-shape` = Chromium-only จึงไม่ใช้ → ใช้ **SVG `<path>` ข้างหลังเนื้อหา** ผ่าน primitive `src/components/ui/Squircle.tsx` + dep `figma-squircle`). ขอบ/สถานะเลือกย้ายจาก CSS border → **SVG stroke** (`fill-*`/`stroke-*` บน path). **ทำแล้ว:** StyleSection (ปุ่มรูปแบบ · `LayerSelector` tiles · `OpeningStyleSelector` tiles = shared 4 ฟอร์ม). **ค้าง/ถัดไป:** roll out ไป `rounded-*` อื่นเป็นเฟส (ปุ่ม→การ์ด→input; การ์ด/input ต้องเพิ่ม non-button variant ใน Squircle). log = memory `squircle-corner-system`. *(gate ยังไม่รัน — เจ้าของ `npm install` figma-squircle ก่อน)*.
