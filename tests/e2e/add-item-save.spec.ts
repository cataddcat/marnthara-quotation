...
test('เพิ่มผ้าม่าน → กรอก กว้าง/สูง → บันทึก & ปิด → เก็บค่า normalize ครบ', async ({ page }) => {
  ...
  await page.getByRole('button', { name: /บันทึก.*ปิด/ }).click();
...
test('กรอกขนาดแล้วแตะ "เปลี่ยน" เลือกประเภทเดิมซ้ำ → ค่าต้องไม่ถูกล้าง (regression ต้นเหตุ)', async ({
  page,
}) => {
  await seed(page);
  await openAddCurtain(page);
  ...
  // แตะชิปประเภทสินค้า (มีป้าย "ประเภทสินค้า … เปลี่ยน") → เลือก "ผ้าม่าน" ซ้ำ
  // — เดิมจะ remount ฟอร์มแล้วล้างค่า
  await page.getByRole('button', { name: /ประเภทสินค้า/ }).click();
  await page.getByRole('button', { name: 'ผ้าม่าน', exact: true }).click();

  // ช่องความสูงต้องยังมีค่า (ไม่ถูกล้าง) — blur ระหว่างทางทำให้ normalize เป็น 2.78
