...
test('แท็บ "สั่งราง" — ชื่อราง/ขนาด(ซม.)/ลูกล้อ N+N/ขาจับ + เตือนขา 2 ชั้น', async ({ page }) => {
  ...
  expect(text).toContain('TES101 ( TW14.5 )รางเทปลอน เทป14.5 สีขาว'); // สีขาว → TES101
  ...
  expect(text).toContain('5 (2ชั้น)'); // ขาจับ = ceil(2.5/0.6)=5, แบบ 2 ชั้น
  expect(text).toContain('รวมรางที่ต้องสั่ง: 2 เส้น'); // 1 ชุด 2ชั้น = 2 เส้น
});

test('แท็บ "สั่งราง" — สลับรูปแบบ "ย่อ" → ขนาด × เส้น · สไลด์ · ขาจับ(ชั้น)', async ({ page }) => {
  await seed(page);
  await openCopySummary(page);
  ...
  expect(text).toContain('250 × 2 เส้น · แยก · ขาจับ (2ชั้น)'); // แบบย่อ
  expect(text).not.toContain('ลูกล้อ'); // แบบย่อไม่โชว์ลูกล้อ/ผ้า
});
...
test('แท็บ "สั่งราง" — จีบ→ราง M (LTL101) · พับ→U-2', async ({ page }) => {
  const mixed = {
    state: {
      rooms: [
        {
  ...
  expect(text).toContain('LTL101 ราง M ประกอบชุด สีขาว'); // จีบ สีขาว → ราง M (LTL101)
